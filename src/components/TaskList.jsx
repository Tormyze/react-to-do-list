      ...
      {props.tasks.map((task) => (
        <li key={task.id}>
          <div className='flex justify-between'>
            <button onContextMenu={(e) => {e.preventDefault(); props.onDetailsClick(task.id)}} onClick={() => {props.onTaskClick(task.id)}}
            ...
            <button onClick={() => props.onDetailsClick(task.id)} className="group">
              {task.expanded ? (
          ...
          <div className="flex justify-between">
            <button className='text-gray-400 text-sm w-full text-start border-l-2 border-transparent pl-2 hover:cursor-pointer' onContextMenu={(e) => {e.preventDefault(); props.onDetailsClick(task.id)}} onClick={() => {props.onTaskClick(task.id)}}>
              {task.expanded && (
                <p>{task.desc}</p>
              )}
            </button>
            <button>
              {task.expanded && (
              <Trash className="text-gray-500 hover:text-red-500 hover:cursor-pointer w-full h-full px-1" />
              )}
            </button>
          </div>
        </li>
